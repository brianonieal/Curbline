...
| `TestEscalationFixture` | 5 | E-030: the demo fixture must hold membership constant and cross every tier, or the advisory ladder cannot be observed |
...
in section 6.2 (E-020 through E-031). Two came from the evidence capture
...
11. **A gauge with a malformed timestamp is dropped rather than reported.**
    `observed_at` is now parsed and required to carry a UTC offset, enforced on
    the `Reading` type so no source can bypass it (E-028). A reading that fails
    is skipped with a log line, so one malformed gauge does not stop the poll
    for the others. The residue is that such a gauge is silently absent from the
    map, and nothing on the console distinguishes it from a sensor that is not
    deployed. This is the same shape as limitation 10 and has the same fix: the
    console needs a state for "reporting but unusable".

12. **The console can still show thresholds the pipeline is not applying.**
    Config is read once per process at import, so restarting a subset of the
    four units leaves the others on the old values. The audit record no longer
    skews this way, because the correlator now carries the parameters it
    clustered with in the message (E-027), but `api/server.py` still reports its
    own copy, and the depth rail and empty-state copy are drawn from it. The
    window is narrow and entirely operator-created, and `DEMO.md` now restarts
    all four units rather than two. Closing it properly means the workers
    publishing their effective configuration and the API reading that rather
    than its own.
...
condition), `ERRORS.md` (31 logged defects), `TESTS.md`, `CHANGELOG.md`,
